...
export class OrderPanel extends BaseComponent {
    ...
    // UI元素
    private ordersList: HTMLElement | null = null;
    private orderDetails: HTMLElement | null = null;
    ...
    protected bindEvents(): void {
        ...
        // 面板折叠按钮
        const toggleBtn = this.element.querySelector('#panelToggle') as HTMLElement;
        if (toggleBtn) {
    ...
    private initializeElements(): void {
        ...
        this.ordersList = this.element.querySelector('#ordersList') as HTMLElement;
        this.orderDetails = this.element.querySelector('#orderDetailsSection') as HTMLElement;
    }

    protected render(): void {
        if (!this.ordersList || !this.orderDetails) return;
    ...
    private createOrderElement(order: Order, index: number): HTMLElement {
        if (!this.ordersList) return document.createElement('div');
    ...
    private showOrderDetails(order: Order): void {
        ...
        // 绑定完成按钮事件
        const completeButton = this.orderDetails.querySelector('.complete-button') as HTMLElement;
        if (completeButton) {
            completeButton.addEventListener('click', () => this.handleCompleteOrder(order));
        }
    }

    private createRequirementElement(item: OrderItem): string {
    ...
    private calculateCompletedItems(order: Order): number {
        // 模拟计算完成的物品数量
        return order.items.filter(() => Math.random() > 0.5).length;
    }

    private handleCompleteOrder(order: Order): void {
    ...
    private updateOrderCount(): void {
        const orderCount = this.element?.querySelector('#orderCount') as HTMLElement;
        if (orderCount) {
    ...
    public showNotification(): void {
        ...
        if (!this.notificationBadge) {
            const panelTitle = this.element?.querySelector('.panel-title') as HTMLElement;
            if (panelTitle) {
    ...
    private togglePanel(): void {
        ...
        const toggle = this.element?.querySelector('#panelToggle') as HTMLElement;
