...
export class GameBoard extends BaseComponent {
    private static readonly GRID_SIZE = 7;
    private static readonly CELL_SIZE = 80; // px

    // 游戏数据
    private gridData: Map<string, ItemRenderData> = new Map();
    private selectedItem: GridPosition | null = null;
    ...
    private handleResponsiveChange(): void {
        ...
        const gridContainer = this.element?.querySelector('#mergeGrid') as HTMLElement;
    ...
    private initializeResponsiveGrid(columns: number): void {
        // 根据列数调整网格大小
    ...
    private bindCellEvents(cell: HTMLElement, position: GridPosition): void {
        // 点击选择
        cell.addEventListener('click', () => this.handleCellClick(position));

        // 拖拽事件
        cell.addEventListener('dragover', (e: any) => this.handleDragOver(e, position));
        cell.addEventListener('drop', (e: any) => this.handleDrop(e, position));
    ...
    private bindControlEvents(): void {
        ...
        const autoArrangeBtn = this.element.querySelector('#autoArrangeBtn') as HTMLElement;
        const clearBoardBtn = this.element.querySelector('#clearBoardBtn') as HTMLElement;
    ...
    protected render(): void {
        if (!this.element || !this.gridElement) return;
    ...
    private renderItem(item: any, position: GridPosition): void {
        ...
        // 绑定拖拽事件
        itemElement.addEventListener('dragstart', (e: any) => this.handleDragStart(e, position));
        itemElement.addEventListener('dragend', (e: any) => this.handleDragEnd(e, position));
    ...
    private handleCellClick(position: GridPosition): void {
        const key = this.getGridKey(position);
    ...
    private handleDragStart(e: DragEvent, position: GridPosition): void {
    ...
    private handleDragEnd(e: DragEvent, position: GridPosition): void {
        const key = this.getGridKey(position);
        const itemElement = this.itemElements.get(key);
    ...
    private handleDragOver(e: DragEvent, position: GridPosition): void {
        e.preventDefault();
        if (!e.dataTransfer) return;
    ...
    private handleDrop(e: DragEvent, position: GridPosition): void {
        e.preventDefault();

        const data = e.dataTransfer?.getData('text/plain');
        if (!data) return;

        const fromPos = JSON.parse(data) as GridPosition;
        this.attemptMerge(fromPos, position);
    }

    private handleDragLeave(position: GridPosition): void {
        const cell = this.cells[position.row][position.col];
        if (cell) {
            cell.classList.remove('drag-over');
        }
    ...
    public selectItem(position: GridPosition): void {
        this.clearSelection();
    ...
    private attemptMerge(fromPos: GridPosition, toPos: GridPosition): void {
        const eventManager = EventManager.getInstance();
    ...
    private updateBoardInfo(itemCount: number): void {
        if (!this.element) return;

        const itemCountElement = this.element.querySelector('.item-count') as HTMLElement;
        const energyDisplay = this.element.querySelector('.energy-display') as HTMLElement;
    ...
    public playGenerateAnimation(): void {
