...
export class StorageManager {
    ...
    private static readonly AUTO_SAVE_INTERVAL = 60000; // 1分钟自动保存
    private static readonly STORAGE_PREFIX = 'feika_save_';
     ...
     * 保存玩家数据到指定槽位
     */
    public async saveToSlot(slotId: string, slotName: string, playerData: any, gameState: string): Promise<boolean> {
        try {
     ...
     * 从指定槽位加载玩家数据
     */
    public async loadFromSlot(slotId: string): Promise<PlayerSaveData | null> {
        try {
     ...
     * 自动保存当前数据
     */
    public async autoSave(playerData: any, gameState: string): Promise<boolean> {
     ...
     * 验证存档数据完整性
     */
    private validateSaveData(saveData: PlayerSaveData): boolean {
        // 检查版本兼容性
     ...
     * 获取存储使用情况
     */
    public getStorageInfo(): { usedSpace: number; totalSpace: number; slots: SaveSlot[] } {
        try {
