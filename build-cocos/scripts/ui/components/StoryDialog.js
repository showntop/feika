...
export class StoryDialog extends BaseComponent {
    ...
    bindEvents() {
        if (!this.element)
            return;
        // 绑定关闭按钮
        const closeBtn = this.element.querySelector('#dialogClose');
        if (closeBtn) {
        ...
        // 绑定跳过按钮
        const skipBtn = this.element.querySelector('#skipBtn');
        if (skipBtn) {
    ...
    initializeElements() {
        ...
        this.speakerElement = this.element.querySelector('#speakerName');
        this.textElement = this.element.querySelector('#dialogText');
        this.nextButton = this.element.querySelector('#nextBtn');
        this.skipButton = this.element.querySelector('#skipBtn');
        this.progressBar = this.element.querySelector('#progressFill');
    }
    render() {
        if (!this.currentEvent || !this.element)
            return;
        // 更新标题
        const titleElement = this.element.querySelector('#dialogTitle');
        if (titleElement) {
    ...
    updateAvatar(speaker) {
        if (!this.element)
            return;
        const avatarElement = this.element.querySelector('#characterAvatar');
        if (!avatarElement)
            return;
        // 根据说话者设置不同的头像
        const avatars = {
    ...
    updateProgress() {
        if (!this.currentEvent || !this.progressBar)
            return;
