...
export class ItemGuideData {
    constructor() {
        this.itemUsageCache = new Map();
        this.generatorGuideCache = new Map();
    }
    static getInstance() {
     ...
     * 计算物品用途信息
     */
    calculateItemUsage(item, allItems, generators, storyEvents) {
        ...
        // 1. 检查合成用途
        const nextLevelItem = allItems.find(i => i.getId() === item.getId() &&
        ...
        generators.forEach(gen => {
            // 通过配置访问production信息
            const production = gen.config?.production;
            if (production && production.items.includes(item.getId())) {
                ...
                const totalWeight = production.weights.reduce((a, b) => a + b, 0);
    ...
    createGeneratorGuide(generator, allItems, storyEvents) {
        ...
        const outputs = production.items.map((itemId, index) => {
        ...
        const guide = {
            ...
            description: generator.config?.description || '生成器',
            outputs,
            cooldown: production.cooldown,
            energyCost: production.energyCost,
            unlockCondition: generator.config?.unlockConditions?.[0]?.value || '第一章解锁'
        };
    ...
    calculateRarity(item) {
        ...
        if (level >= 5 || value >= 3000)
            return 'epic';
        if (level >= 4 || value >= 1000)
            return 'rare';
        if (level >= 3 || value >= 200)
            return 'uncommon';
        return 'common';
    }
    /**
...
//# sourceMappingURL=ItemGuideData.js.map